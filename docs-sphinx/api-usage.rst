...
dohq-teamcity supports both GitLab v3 and v4 APIs.
...
``teamcity.Gitlab`` class
=======================

To connect to a GitLab server, create a ``teamcity.Gitlab`` object:
...
.. code-block:: python
   ...
   gl = teamcity.Gitlab('http://10.0.0.1', private_token='JVNSESs8EwWRx5yDxM5q')

   # oauth token authentication
   gl = teamcity.Gitlab('http://10.0.0.1', oauth_token='my_long_token_here')

   # username/password authentication (for GitLab << 10.2)
   gl = teamcity.Gitlab('http://10.0.0.1', email='jdoe', password='s3cr3t')

   # anonymous teamcity instance, read-only for public resources
   gl = teamcity.Gitlab('http://10.0.0.1')

   # make an API request to create the gl.user object. This is mandatory if you
   # use the username/password authentication.
   gl.auth()

You can also use configuration files to create ``teamcity.Gitlab`` objects:

.. code-block:: python

   gl = teamcity.Gitlab.from_config('somewhere', ['/tmp/gl.cfg'])
...
been removed from GitLab in version 10.2, and is not available on teamcity.com
...
The ``teamcity.Gitlab`` class provides managers to access the GitLab resources.
...
* ``teamcity.Gitlab`` is the primary class, handling the HTTP requests. It holds
  the GitLab URL and authentication information.
* ``teamcity.base.RESTObject`` is the base class for all the GitLab v4 objects.
...
.. code-block:: python
   ...
   gl = teamcity.Gitlab(url, token, per_page=50)
...
.. code-block:: python
   ...
   with teamcity.Gitlab(host, token) as gl:
       gl.projects.list()
...
dohq-teamcity obeys the rate limit of the GitLab server by default.  On
